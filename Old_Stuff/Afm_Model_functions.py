...
############################################################################################################
def H_afm(L, J1, J2, Dxy, d, dprime, make_stoquastic=False, return_space=False):
    """
    inputs:
...
# def H_afm(L, J1, J2, Dxy, d, dprime, make_stoquastic=False, J1_0_limit = False, J2_0_limit=False, return_space=False):
#     """
#     inputs:
#     L: linear dimension along the basis vector [1,0] and vector [1/2, sqrt(3)/2] of the triangular lattice
#     J1: interaction strength along the horizontal spin chains (even and odd)
...
#     make_stoquastic: (bool) if True, the Hamiltonian is stoquastic, i.e., off diagonal elements are non-positive
#     J1_0_limit: (bool) if True, the Hamiltonian is zero along the horizontal spin chains
#     J2_0_limit: (bool) if True, the Hamiltonian is zero along the diagonal spin chains (i.e. zig zag bonds)
#     return_space: (bool) if True, the function returns the hilbert space as well
...
#     if d == 0. and Dxy == 0. and dprime == 0. and make_stoquastic:
#         if J1_0_limit:
#             bond_operator = [ (Jeven[0] * SxSx).tolist(), #Jeven is anyway zero in that limit!
#                             (Jeven[1] * SySy).tolist(),
#                             (Jeven[2] * SzSz).tolist(),
#                             (Jodd[0] * SxSx).tolist(), #Jodd is anyway zero in that limit!
#                             (Jodd[1] * SySy).tolist(),
...
#                         ]
#         elif J2_0_limit:
#             bond_operator = [ (-Jeven[0] * SxSx).tolist(),
#                             (-Jeven[1] * SySy).tolist(),
#                             (Jeven[2] * SzSz).tolist(),
#                             (-Jodd[0] * SxSx).tolist(),
#                             (-Jodd[1] * SySy).tolist(),
#                             (Jodd[2] * SzSz).tolist(),
#                             (Jprime[0] * SxSx).tolist(), #Jprime is anyway zero in that limit!
#                             (Jprime[1] * SySy).tolist(),
#                             (Jprime[2] * SzSz).tolist(),  
#                         ]
#         else:
...
def H_afmJ123(L, J1, J2, J3, Dxy, d, dprime, return_space=False, sublattice = list, make_rotation=False, exchange_XY=False):
    """
    inputs:
    L: linear dimension along the basis vector [1,0] and vector [1/2, sqrt(3)/2] of the triangular lattice
    J1: interaction strength along the horizontal spin chains (even and odd)
...
####################################################################################################################
# def H_afmJ123(L, J1, J2, J3, Dxy, d, dprime, return_space=False, make_stoquastic=False, exchange_XY=False ):
#     """
#     inputs:
#     L: linear dimension along the basis vector [1,0] and vector [1/2, sqrt(3)/2] of the triangular lattice
#     J1: interaction strength along the horizontal spin chains (even and odd)
...
#     make_stoquastic: (bool) if True, the Hamiltonian is stoquastic, i.e., off diagonal elements are non-positive
#     Remark: the Hamiltonian is only really stoquastic if J_3=0 otherwise it is quasi stoquastic!!
...
#     bond_operator = [
#         ((-1)**n * Jeven[0] * SxSx).tolist(),
#         ((-1)**n * Jeven[1] * SySy).tolist(),
#         (Jeven[2] * SzSz).tolist(),
#         ((-1)**n * Jodd[0] * SxSx).tolist(),
#         ((-1)**n * Jodd[1] * SySy).tolist(),
#         (Jodd[2] * SzSz).tolist(),
#         ((-1)**n * J2diag[0] * SxSx).tolist(),
#         ((-1)**n * J2diag[1] * SySy).tolist(),
#         (J2diag[2] * SzSz).tolist(), 
#         (J3diag[0] * SxSx).tolist(),
#         (J3diag[1] * SySy).tolist(),
#         (J3diag[2] * SzSz).tolist(),  
#         ]   



#     bond_color = [0,0,0,1,1,1,2,2,2,3,3,3]
...
def H_afm_1d(L, J1, Dxy, d, even=True, make_rotation=False, exchange_XY = False, return_space=False):
    """
    inputs:
    L: linear dimension 
    J1: interaction strength along the horizontal spin chains (even and odd)
