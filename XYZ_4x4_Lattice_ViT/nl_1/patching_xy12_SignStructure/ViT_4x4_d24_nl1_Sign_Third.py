...
p_opt = {
    'learning_rate' : 1e-3,#linear_schedule(init_value=1e-2, end_value=1e-2, transition_steps=100),
...
gparams = {'params': {'Vit_2d_full_symm_0': {'ViT_2d_0': params['params']}}}

translations = vit.get_translation(L,L,L**2, L,L)
symms = nk.graph.Graph(edges = make_colored_edges(L,L)).automorphisms().to_array()
rest = vit.get_difference(symms, translations)
Id = np.arange(0,L**2)
Reflec = vit.make_first_reflection(Id, L)
# GlideRot = vit.rot180_trans1(Id, L)
# ReflecGlide = vit.trans_product(GlideRot, Reflec)
reflecs_arr = HashableArray(jnp.array([Id, Reflec]))
...
#                 # define the model
    ...
    StateLogger = PickledJsonLog(output_prefix=DataDir + 'log_vit_sampler_{}_sign_refined_flip_reflection'.format(sa_key), save_params_every=10, save_params=True)
