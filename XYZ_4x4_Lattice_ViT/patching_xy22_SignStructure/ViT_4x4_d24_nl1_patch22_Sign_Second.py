...
p_opt = {
    ...
    # 'diag_shift': 1e-4,
    'diag_shift': linear_schedule(init_value=1e-5, end_value=1e-3, transition_begin=150, transition_steps=150),
...
pVit = {
    ...
    'Cx': 2,
    'Cy': 2,
    'patch_arr': HashableArray(jnp.array([[0,1,4,5], [2,3,6,7], [8,9,12,13], [10,11,14,15]])),
}
...
with open('Log_Files/log_vit_sampler_HaEx_5050_sign.pickle', 'rb') as handle:
...
#                 # define the model
    ...
    StateLogger = PickledJsonLog(output_prefix=DataDir + 'log_vit_sampler_{}_transflip_sign'.format(sa_key), save_params_every=10, save_params=True)
