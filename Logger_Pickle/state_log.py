...
class PickledStateLog(AbstractLog):
    """
    ...
    def _save_variables(self, variational_state):
        ...
        # )
        binary_data = pickle.dumps(variational_state.variables)

        if self._is_master_process:
            if self._tar:
                save_binary_to_tar(
                    self._tar_file, binary_data, str(self._file_step) + ".pickle"
                )
            else:
                with open(self._prefix + str(self._file_step) + ".pickle", "wb") as f:
                    pickle.dump(variational_state.variables, f)
