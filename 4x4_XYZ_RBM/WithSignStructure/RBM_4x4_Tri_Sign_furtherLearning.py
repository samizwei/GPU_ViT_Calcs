...
p_opt = {
    ...
    # 'diag_shift': 1e-3,
    'diag_shift': linear_schedule(init_value=1e-3, end_value=1e-5, transition_begin=0, transition_steps=150),
    'n_samples': 2**12,
    'chunk_size': 2**12,
    'n_iter': 250,
}
...
for j, alpha in enumerate(alphas):
    ...
    with open('/scratch/samiz/4x4_XYZ_RBM/WithSignStructure/Log_Files/log_saHaEx5050_rbm_alpha{}_sign.pickle'.format(alpha), 'rb') as handle:
        params = pickle.load(handle)
    ...
    StateLogger = PickledJsonLog(output_prefix=path_to_store + 'log_saHaEx5050_rbmtransflip_alpha{}_sign'.format(alpha), save_params_every=10, save_params=True)
