...
p_opt = {
    ...
    # 'diag_shift': 1e-3,
    'diag_shift': linear_schedule(init_value=1e-4, end_value=1e-5, transition_steps=100, transition_begin=200),
    'n_samples': 2**12,
    'chunk_size': 2**12,
    'n_iter': 300,
}
...
with open(DataDir + 'log_vit_sampler_HaEx_5050.pickle', 'rb') as f:
...
#                 # define the model
    ...
    StateLogger = PickledJsonLog(output_prefix=DataDir + 'log_vit_sampler_{}_refined_flip'.format(sa_key), save_params_every=10, save_params=True)
