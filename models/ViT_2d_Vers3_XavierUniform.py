...
- Additionally we use a Pre Layer Normalization in the Final Complex Layer
- In the Feed Forward Network we use the activation function gelu
"""
...
class Cell_Patching(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.Dense(features=self.embed_dim, kernel_init=xavier_uniform(), param_dtype=self.Dtype)(x)
...
class Self_Attention_2d(nn.Module):
    ...
    def Sample_Attention(self, x):
        ...
        alpha = self.param('alpha', xavier_uniform(), (self.num_heads, num_patches), dtype = self.Dtype)
        V = self.param('V_kernel', xavier_uniform(), (self.num_heads, r, pre_embed_dim), dtype = self.Dtype)
        ...
        y = nn.Dense(features=self.num_heads, kernel_init = xavier_uniform(), name = 'head_mixing',
...
class FullConn_2_Layer(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.Dense(features=self.dense_dim * 2, kernel_init=xavier_uniform(), name='Dense_d_to_2d',
        ...
        x = nn.Dense(features=self.dense_dim, kernel_init=xavier_uniform(), name='Dense_2d_to_d', 
...
class ViT_2d(nn.Module):
    ...
    def __call__(self, x):
        ...
        rbm = Final_Complex_Layer(hidden_density=self.hidden_density, param_dtype=self.Dtype, use_visible_bias=True)
