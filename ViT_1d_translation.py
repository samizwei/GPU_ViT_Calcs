...
#         x = nn.Dense(features=self.embed_dim, name='Dense_PreEncode', param_dtype=self.Dtype)(x)
       
#         assert x.shape == (n_samples, num_patches, self.embed_dim), 'wrong shape of output x'
...
class Simplified_SelfAttention(nn.Module):
    ...
    def Sample_Attention(self, x):
        ...
        alpha = self.param('alpha', nn.initializers.normal(), (self.heads, num_patches), dtype = self.Dtype)  # (heads, num_patches)
        V = self.param('V', nn.initializers.normal(), (self.heads, r, patch_size), dtype = self.Dtype)  # (heads, r, embed_dim)
    ...
    def __call__(self,x):
        ...
        # apply Dense layer
        y = nn.Dense(features=self.embed_dim, dtype=self.Dtype, param_dtype=self.Dtype)(y)
