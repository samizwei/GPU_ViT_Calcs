...
p_opt = {
    ...
    # 'diag_shift': 1e-4, #1e-3
    'diag_shift': linear_schedule(init_value=1e-4, end_value=1e-3, transition_begin=50, transition_steps=100),
    'n_samples': 2**12,
    'chunk_size': 2**12,
    'n_iter': 300,
}
...
for j, alpha in enumerate(alphas):
    ...
    with open('/scratch/samiz/6x6_XYZ_RBM/Log_Files/log_saHaEx5050_rbm_alpha{}.pickle'.format(alpha), 'rb') as handle:
        params = pickle.load(handle)
    ...
    StateLogger = PickledJsonLog(output_prefix=path_to_store + 'log_saHaEx5050_rbmtransflip_alpha{}'.format(alpha), save_params_every=10, save_params=True)
