...
# decay_schedule = linear_schedule(init_value=2*1e-3, end_value=1e-4, transition_begin=150, transition_steps=200)

# lr_schedule = join_schedules(schedules=[warmup_schedule, decay_schedule], boundaries=[50] )

p_opt = {
    # 'learning_rate' : 1e-2,
    'learning_rate' : linear_schedule(init_value=0.5 * 1e-2, end_value = 0.5 * 1e-4, transition_begin=100, transition_steps=300),
    # 'learning_rate': cosine_decay_schedule(init_value=1e-3, decay_steps = 100, alpha = 1e-2),
    # 'diag_shift': 1e-4,
    'diag_shift': linear_schedule(init_value=1e-3, end_value=1e-6, transition_begin=100, transition_steps=300),
    'n_samples': 2**12,
    'chunk_size': 2**12,
    'n_iter': 500,
}
...
with open('/scratch/samiz/GPU_ViT_Calcs/XYZ_8x8_Lattice_ViT/patching_xy44/Log_Files/log_vit_sampler_HaEx_5050.pickle', 'rb') as f:
    ps = pickle.load(f)
...
#                 # define the model
    m_Vit = vit.Vit_2d_full_symm(patch_arr=HashableArray(pVit['patch_arr']), embed_dim=pVit['d'], num_heads=pVit['h'], nl=pVit['nl'],
                                ...
                                recover_full_transl_symm=True, translations = patch_transl, recover_spin_flip_symm=False)
    ...
    StateLogger = PickledJsonLog(output_prefix=DataDir + 'log_vit_sampler_{}_trans'.format(sa_key), save_params_every=10, save_params=True)
