...
from flax.linen.initializers import xavier_uniform, xavier_normal, variance_scaling

cust_kernel_init = variance_scaling(scale=0.01, mode= 'fan_in', distribution='truncated_normal')
...
class Cell_Patching(nn.Module):
    ...
    Dtype: jnp.dtype
    kern_init : NNInitFunc = cust_kernel_init
    """
    args:
...
class Final_Complex_Layer(nn.Module):
    ...
    kernel_init: NNInitFunc = xavier_uniform()# default_kernel_init #xavier_uniform()#default_kernel_init
    ...
    def __call__(self, x):
        ...
        y_real = (jnp.matmul(x, W_real) + b_real) #<----------------------------------- try out Layer Norm here
...
class ViT_2d(nn.Module):
    ...
    kern_init_rbm : NNInitFunc = cust_kernel_init
    kern_init_vit : NNInitFunc = xavier_uniform()
