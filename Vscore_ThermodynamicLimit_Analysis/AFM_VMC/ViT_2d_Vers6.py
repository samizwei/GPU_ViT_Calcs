...
"""
In this version we initialize the parameters belonging to a kernel with xavier_unform() and all the biases with zeros (jax.nn.initializers.zeros)
Moreover the kernel init for the rbm part is given by varianc_scaling with sclae=0.01
"""
...
class Cell_Patching(nn.Module):
    ...
    Dtype: jnp.dtype
    kern_init : NNInitFunc = transformer_kernel_init
    """
    args:
    ...
    def __call__(self, x):
        ...
        x = nn.Dense(features=self.embed_dim, kernel_init=self.kern_init, param_dtype=self.Dtype)(x)
...
class Self_Attention_2d(nn.Module):
    ...
    def Sample_Attention(self, x):
        ...
        alpha = self.param('alpha', self.kern_init, (self.num_heads, num_patches), dtype = self.Dtype)
        V = self.param('V_kernel', self.kern_init, (self.num_heads, r, pre_embed_dim), dtype = self.Dtype)
        V_bias = self.param('V_bias', jax.nn.initializers.zeros, (self.num_heads, r, 1), dtype = self.Dtype) #the one in shape makes sure that the bias is broadcasted correctly and constant along the patch axis
        ...
        y = nn.Dense(features=self.num_heads, kernel_init = self.kern_init, name = 'head_mixing',
...
class FullConn_2_Layer(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.Dense(features=self.dense_dim * 2, kernel_init=self.kern_init, name='Dense_d_to_2d',
        ...
        x = nn.Dense(features=self.dense_dim, kernel_init=self.kern_init, name='Dense_2d_to_d', 
...
class Transformer_Encoder(nn.Module):
    ...
    def __call__(self, x):
        ...
        x1 = Self_Attention_2d(num_heads=self.num_heads, embed_dim=self.embed_dim, L = self.L, Cx=self.Cx, Cy=self.Cy, kern_init=self.kern_init, Dtype = self.Dtype)(x1) + x

        x2 = nn.LayerNorm(param_dtype=self.Dtype)(x1)
        # x2.shape = (n_samples, num_patches, embed_dim)
        #non linearity for each patch!
        non_lin = FullConn_2_Layer(dense_dim=self.embed_dim, Dtype = self.Dtype, activation = self.Twolayer_activation, kern_init=self.kern_init)
        x2 = non_lin(x2) + x1
...
class ViT_real(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = Cell_Patching(embed_dim=self.embed_dim, patch_array = self.patch_arr, Dtype = self.Dtype, kern_init=self.kern_init)(x)


        for _ in range(self.nl):
            x = Transformer_Encoder(num_heads=self.num_heads, embed_dim=self.embed_dim, L=self.L,
                                     Cx=self.Cx, Cy=self.Cy, Dtype = self.Dtype, kern_init=self.kern_init, Twolayer_activation = self.Twolayer_activation)(x)
...
class Final_Complex_Layer(nn.Module):
    ...
    kernel_init: NNInitFunc = rbm_kernel_init# default_kernel_init #xavier_uniform()#default_kernel_init
    """Initializer for the Dense layer matrix."""
    hidden_bias_init: NNInitFunc = jax.nn.initializers.zeros#default_kernel_init
    """Initializer for the hidden bias."""
    visible_bias_init: NNInitFunc = jax.nn.initializers.zeros#default_kernel_init
    ...
    def __call__(self, x):
        ...
        x = nn.LayerNorm(param_dtype=self.param_dtype)(x)   #<----------------------------------- try out Layer Norm here
        ...
        # y_real = nn.LayerNorm(param_dtype = self.param_dtype)(jnp.matmul(x, W_real) + b_real) #<----------------------------------- try out Layer Norm here
        # y_imag = nn.LayerNorm(param_dtype = self.param_dtype)(jnp.matmul(x, W_imag) + b_imag)

        y_real = (jnp.matmul(x, W_real) + b_real) #<----------------------------------- No LayerNorm here!!
        y_imag = (jnp.matmul(x, W_imag) + b_imag)
        y = y_real + 1j * y_imag
        ...
        # y_real = nn.Dense(features=embed_dim, param_dtype=self.param_dtype, kernel_init=self.kernel_init,
        #                   bias_init=self.hidden_bias_init )(x)

        # y_imag = nn.Dense(features=embed_dim, param_dtype=self.param_dtype, kernel_init=self.kernel_init,
        #                   bias_init=self.hidden_bias_init )(x)

        # y = y_real + 1.0j * y_imag

        # return jnp.sum(self.activation(y), axis=-1)
...
class ViT_2d(nn.Module):
    ...
    kern_init_rbm : NNInitFunc = rbm_kernel_init
    kern_init_vit : NNInitFunc = transformer_kernel_init
    use_rbm_visible_bias : bool=False
    use_rbm_hidden_bias : bool=True
    TwoLayer_activation: Any = nn.gelu
    ...
    def __call__(self, x):
        ...
        vit = ViT_real(patch_arr = self.patch_arr, embed_dim = self.embed_dim, num_heads = self.num_heads, nl = self.nl,
                           Dtype = self.Dtype, L = self.L, Cx = self.Cx, Cy = self.Cy, kern_init=self.kern_init_vit, Twolayer_activation = self.TwoLayer_activation)
                          
        rbm = Final_Complex_Layer(hidden_density=self.hidden_density, param_dtype=self.Dtype,
                                   use_hidden_bias=self.use_rbm_hidden_bias, use_visible_bias=self.use_rbm_visible_bias, kernel_init=self.kern_init_rbm)
...
class Vit_2d_full_symm(nn.Module):
    """
    info: only translation and spin flip symmetry can be recovered!!!
...
# we forgot reflection symmetry so here we go:

class Vit_2d_add_reflection(nn.Module):
    """
    info:
    ...
    reflections: jnp.ndarray=None


    @nn.compact
    def __call__(self, x):
        vit_model=Vit_2d_full_symm(patch_arr = self.patch_arr, embed_dim = self.embed_dim, num_heads = self.num_heads, nl = self.nl,
                        Dtype = self.Dtype, L = self.L, Cx = self.Cx, Cy = self.Cy, hidden_density = self.hidden_density, TwoLayer_activation = self.TwoLayer_activation,
                        recover_full_transl_symm=self.recover_full_transl_symm, translations=self.translations,
                        recover_spin_flip_symm=self.recover_spin_flip_symm)
        
        # z = jnp.apply_along_axis(lambda trans_elt: jnp.array([vit_model(x[..., trans_elt]), vit_model((-1)*x[..., trans_elt])]), axis = -1, arr=jnp.asarray(self.reflections))
        # z = z.reshape(-1, z.shape[-1])
         
        # return logsumexp_cplx(z, axis=0)
        z = jnp.apply_along_axis(lambda trans_elt: vit_model(x[..., trans_elt]),
                                                 axis = -1,
                                                 arr = jnp.asarray(self.reflections))
